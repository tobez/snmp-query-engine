...
int
build_get_request_packet(int version, const char *community, const char *oid_list, unsigned request_id, struct ber *e)
{
    ...
    if (encode_integer((unsigned)version, e, 0) < 0)
        return -1;
    if (encode_string(community, e) < 0)
        return -1;
    ...
    while (*oid_list) {
        ...
        e->b[1] = 0;
        EXTEND2;
        if (encode_store_length(e, seq) < 0)
            return -1;
        oid_list += l + 1;
    }

    if (encode_store_length(e, oid_sequence) < 0)
        return -1;
...
int
start_snmp_packet(struct packet_builder *pb, int version, unsigned request_id, const struct snmpv3info *v3, const char *community)
{
    ...
    if (version == 3) {
        ...
        if (encode_bytes(&msg_flags, 1, e) < 0)
            return -1;
        if (encode_integer(msg_security_model, e, 0) < 0)
            return -1;
        ...
        if (encode_bytes(v3->engine_id, v3->engine_id_len, e) < 0)
            return -1;
...
int
finalize_snmp_packet(struct packet_builder *pb, struct ber *out_encoded_packet, const struct snmpv3info *v3,
                     struct packet_info *out_pi, unsigned char type, int max_repetitions)
{
    struct ber *e;
    int         l;
    e = &pb->e;
...
{
    int i = strlen(s);
    if (encode_type_len(AT_STRING, i, e) < 0)
        return -1;
...
{
    if (encode_type_len(AT_STRING, n, e) < 0)
        return -1;
...
{
    unsigned char t;
    if (decode_type_len(e, &t, s_len) < 0)
        return -1;
    if (t != AT_STRING) {
        errno = EINVAL;
        return -1;
...
oid_belongs_to_table(struct ber *oo, struct ber *tt)
{
    ...
    if (decode_type_len(&o, &otype, &olen) < 0)
        return 0;
    if (decode_type_len(&t, &ttype, &tlen) < 0)
        return 0;

    if (olen <= tlen)
        return 0;
    if (otype != AT_OID || ttype != AT_OID)
        return 0;

    if (memcmp(o.b, t.b, tlen) != 0)
        return 0;
    return 1;
}
...
oid_compare(struct ber *aa, struct ber *bb)
{
    ...
    if (decode_type_len(&a, &atype, &alen) < 0)
        return PROBLEM;
    if (decode_type_len(&b, &btype, &blen) < 0)
        return PROBLEM;
    if (atype != AT_OID || btype != AT_OID)
        return PROBLEM;
